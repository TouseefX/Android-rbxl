...
impl TextCursorState {
    ...
    pub fn pointer_interaction(
        ...
        if response.long_touched() {
            // Android/iOS convention: a stationary hold enters selection mode.
            // Touch TextEdits intentionally use click-only Sense so ordinary
            // drags can pan an enclosing ScrollArea.
